...
`;
  ...
  public async getChatResponse(
    ...
    try {
      // If no client, call Firebase function instead (safer way to handle API keys)
      if (!this.client) {
        const callAIFunction = httpsCallable<
          { messages: Message[]; goals: Goal[]; motivations: Motivation[] },
          { response: string }
        >(this.functions, 'callOpenAI');
  ...
  public async extractInsights(conversation: Conversation): Promise<{
    ...
    try {
      if (!this.client) {
        const extractInsightsFunction = httpsCallable<
          { conversation: Conversation },
          { motivations: string[]; keyMoments: string[] }
        >(this.functions, 'extractInsights');
        
        const result = await extractInsightsFunction({ conversation });
        return result.data;
      }
...
`;
  ...
  public async suggestMilestones(goalTitle: string, goalDescription?: string): Promise<string[]> {
    try {
      if (!this.client) {
        const suggestMilestonesFunction = httpsCallable<
          { goalTitle: string; goalDescription?: string },
          { milestones: string[] }
        >(this.functions, 'suggestMilestones');
        
        const result = await suggestMilestonesFunction({ 
          goalTitle, 
          goalDescription 
        });
...
`;
  ...
  public async getCoachingTip(
    ...
    try {
      if (!this.client) {
        const getCoachingTipFunction = httpsCallable<
          { userId: string; goals: Goal[]; recentMood?: string },
          { tip: string }
        >(this.functions, 'getCoachingTip');
        
        const result = await getCoachingTipFunction({ 
          userId, 
          goals,
          recentMood 
        });
